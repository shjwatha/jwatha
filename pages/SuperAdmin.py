...
if selected_tab == "إدارة الأعضاء":
    ...
    with st.form("add_admin"):
        ...
        if submit_admin:
            cursor.execute("SELECT 1 FROM admins WHERE full_name = %s OR username = %s", (username, full_name))
            conflict_admin = cursor.fetchone()
            cursor.execute("SELECT 1 FROM users WHERE full_name = %s OR username = %s", (username, full_name))
            conflict_user = cursor.fetchone()
...
# ========== التبويب الثالث: نقاطي ==========
    ...
    with st.form("add_supervisor_criterion"):
        ...
        visible_choice = st.selectbox("📢 هل يظهر للمستخدم؟", ["نعم", "لا"], key="new_visibility")
        is_visible = True if visible_choice == "نعم" else False
        submitted = st.form_submit_button("➕ أضف البند")

        if submitted and question:
            try:
                insert_query = "INSERT INTO supervisor_criteria (question, max_score, level, is_visible_to_user) VALUES (%s, %s, %s, %s)"
                cursor.execute(insert_query, (question, max_score, level, is_visible))
                conn.commit()
                ...
                with st.expander(f"{row['question']} (درجة كاملة: {row['max_score']})"):
                    ...
                    with col1:
                        with st.form(key=f"edit_form_{row['id']}"):
                            ...
                            vis_choice = st.selectbox("📢 هل يظهر للمستخدم؟", ["نعم", "لا"], index=0 if row.get("is_visible_to_user", True) else 1, key=f"vis_{row['id']}")
                            new_visible = True if vis_choice == "نعم" else False

                            if st.form_submit_button("📝 تحديث"):
                                try:
                                    update_query = "UPDATE supervisor_criteria SET question = %s, max_score = %s, is_visible_to_user = %s WHERE id = %s"
                                    cursor.execute(update_query, (new_question, new_score, new_visible, row['id']))
                                    conn.commit()
                                    st.success("✅ تم التحديث")
                                    st.rerun()
                                except Exception as e:
...
elif selected_tab == "إدارة مسميات الإنجاز":
    st.header("🎯 إدارة مسميات الإنجاز")

    st.subheader("➕ إضافة إنجاز جديد")
    with st.form("add_achievement_form"):
        new_achievement = st.text_input("🏆 اسم الإنجاز الجديد")
        add_button = st.form_submit_button("✅ أضف الإنجاز")

        if add_button and new_achievement.strip():
            try:
                cursor.execute("INSERT INTO achievements_list (achievement) VALUES (%s)", (new_achievement.strip(),))
                conn.commit()
                st.success("✅ تم إضافة الإنجاز بنجاح.")
                st.rerun()
            except Exception as e:
                st.error(f"❌ حدث خطأ أثناء إضافة الإنجاز: {e}")
        elif add_button:
            st.warning("⚠️ يرجى إدخال اسم الإنجاز.")

    st.subheader("📋 قائمة الإنجازات الحالية")
    try:
        cursor.execute("SELECT id, achievement FROM achievements_list")
        achievements = cursor.fetchall()
        if not achievements:
            st.info("ℹ️ لا توجد إنجازات حتى الآن.")
        else:
            for ach in achievements:
                col1, col2, col3 = st.columns([3, 1, 1])
                with col1:
                    updated_name = st.text_input("🏷️ اسم الإنجاز", value=ach["achievement"], key=f"ach_{ach['id']}")
                with col2:
                    if st.button("📝 تعديل", key=f"edit_{ach['id']}"):
                        try:
                            cursor.execute("UPDATE achievements_list SET achievement = %s WHERE id = %s", (updated_name.strip(), ach["id"]))
                            conn.commit()
                            st.success("✅ تم تعديل الإنجاز.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ فشل في تعديل الإنجاز: {e}")
                with col3:
                    if st.button("🗑️ حذف", key=f"del_{ach['id']}"):
                        try:
                            cursor.execute("DELETE FROM achievements_list WHERE id = %s", (ach["id"],))
                            conn.commit()
                            st.success("✅ تم حذف الإنجاز.")
                            st.rerun()
                        except Exception as e:
                            st.error(f"❌ فشل في حذف الإنجاز: {e}")
    except Exception as e:
